...
from isaaclab.utils.math import combine_frame_transforms, transform_points, euler_xyz_from_quat
...
def shelf_collision_termination(env: ManagerBasedRLEnv,
    ...
    shelf_vel.sum()
    # print(f"l: {dst_l_shelf}")
    # print(f"r: {dst_r_shelf}")
...
def hand_velocity_termination(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), threshold: float = 0.1):
    ...
    # joint_vel = robot.data.joint_vel[:, :6].clone()
