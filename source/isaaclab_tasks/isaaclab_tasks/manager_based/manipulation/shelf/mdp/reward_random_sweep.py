...
from isaaclab_tasks.manager_based.manipulation.shelf.src.shelf_utils import (
    normalize_angle,
)
...
def reward_for_hand_reaching(
    ...
    target_pos_w = object_collection.data.object_pos_w[
        torch.arange(env.scene.num_envs), target_ids
    ].clone()
    ...
    distance_wrist = torch.norm(
        (offset_pos[:, 1:3] - wrist_pos_w[..., 0, 1:3]), dim=-1, p=2
    )
...
def align_ee_target(
    ...
    align_y_z = (
        torch.bmm(ee_tcp_y.unsqueeze(1), shelf_z.unsqueeze(-1)).squeeze(-1).squeeze(-1)
    )
...
def pushing_target(
    ...
    target_lin_vel_w = object_collection.data.object_lin_vel_w[
        torch.arange(env.scene.num_envs), target_ids
    ]
    ...
    zeta_m = torch.where(
        (torch.norm(offset_pos - ee_pos_w, dim=-1, p=2)) < 0.04,
        torch.where(torch.abs(offset_pos[:, 1] - wrist_pos_w[:, 1]) < 0.04, 1, 0),
        0,
    )
    obj_vel_rew = torch.where(
...
class shelf_Collision(ManagerTermBase):
    ...
    def shelf_dynamic_penalty(
        ...
        distance = torch.norm(
            shelf_pos_w - self._ee.data.target_pos_w[..., 0, :], dim=-1, p=2
        )
